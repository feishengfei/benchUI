...
/**
 * @file copytexsubimage3d.cpp
 * @author Otto.Liu
 * @par History:
 *    A001: Otto.Liu, 2019-09-02, Add commit
 * @brief basic test for copytexsubimage3d related API
 * @par TestPoints:
 *
 */
...
void setNextTextureImage(GLvoid* textureData) {
    GL_CHECK(glCopyTexSubImage3D(GL_TEXTURE_3D, 0, 0, 0, textureZOffset,
...
void loadImages(void) {
    for (int currentImageIndex = 1; currentImageIndex <= imagesCount;
        ...
        const string filePath = (string)GLSL_PATH + imagesFilename + "." + numericExtension;
...
bool initializeAttribArrays(void) {
    ...
    GLint inputUVWCoordinatesLocation = GL_CHECK(glGetAttribLocation(programID,
            "inputUVWCoordinates"));
    ...
    if (inputUVWCoordinatesLocation != -1) {
        ...
        GL_CHECK(glBufferData(GL_ARRAY_BUFFER, sizeof(uvwCoordinates),
                              uvwCoordinates, GL_STATIC_DRAW));
        GL_CHECK(glVertexAttribPointer(inputUVWCoordinatesLocation, 3,
...
void initializeUniformData(void) {
    ...
    if (cameraMatrixLocation != -1) {
        ...
        GL_CHECK(glUniformMatrix4fv(cameraMatrixLocation, 1, GL_FALSE,
                                    cameraMatrix.getAsArray()));
    } else {
        LOGE("Could not locate \"cameraMatrix\" uniform in program [%d].", programID);
    }

    if (projectionMatrixLocation != -1) {
        Matrix projectionMatrix = Matrix::matrixPerspective(45.0f,
                (float) window_Width / (float) window_Height, 0.01f, 10.0f);
        GL_CHECK(glUniformMatrix4fv(projectionMatrixLocation, 1, GL_FALSE,
                                    projectionMatrix.getAsArray()));
    } else {
        LOGE("Could not locate \"projectionMatrix\" uniform in program [%d].", programID);
    }

    if (textureSamplerLocation != -1) {
        GL_CHECK(glUniform1i(textureSamplerLocation, 0));
    } else {
        LOGE("Could not locate \"textureSampler\" uniform in program [%d].", programID);
    }

    if (instancesCountLocation != -1) {
        GL_CHECK(glUniform1i(instancesCountLocation, textureDepth));
    } else {
        LOGE("Could not locate \"instancesCount\" uniform in program [%d].", programID);
    }

    if (minBlendingThresholdLocation != -1) {
        GL_CHECK(glUniform1f(minBlendingThresholdLocation, minBlendingThreshold));
    } else {
        LOGE("Could not locate \"minBlendingThreshold\" uniform in program [%d].", programID);
    }

    if (isMinBlendingLocation == -1) {
        LOGE("Could not locate \"isMinBlending\" uniform in program [%d]. Blending equation will not change.", programID);
    }

    if (rotationVectorLocation == -1) {
        LOGE("Could not locat \"rotationVector\" uniform in program [%d]. Texture will not rotate.", programID);
    }
}
...
void setBlendEquation(GLenum isMinBlending) {
    if (isMinBlending) {
