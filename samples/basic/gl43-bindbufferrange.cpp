...
bool initializeData() {
    ...
    getTriangleRep(cube.scalingFactor, &cube.numberOfElementsInCoordinatesArray, &cube.coordinates);
    return true;
}
...
bool setupCubesAndPlaneProgram(void) {
    ...
    setUpAndUseProgramObject(cubesAndPlaneProgram.programId, "BindBufferRange.frag",
                             "BindBufferRange.vert");

    cubesAndPlaneProgram.positionAttributeLocation =
    GL_CHECK(glGetAttribLocation(cubesAndPlaneProgram.programId, "attributePosition"));
    ...
    GLint offset[2];
    glGetActiveUniformsiv(cubesAndPlaneProgram.programId, 2, indices,
...
void renderFrame(void) {
    ...
    glBindBuffer(GL_UNIFORM_BUFFER, uboHandle);
    glBindBufferRange(GL_UNIFORM_BUFFER, cameraPosBlockIndex, uboHandle, 0, blockSize);
