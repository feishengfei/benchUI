...
bool setupGraphics(int width, int height)
{
    ...
    string vertex = ((string)GLSL_PATH) + "vert4_passby_location.vert"; 
...
void renderFrame(void)
{
    ...
    GL_CHECK(glBindVertexArray(VertexArrayName[buffer::FEEDBACK1]));
    GL_CHECK(glDrawTransformFeedbackStream(GL_POINTS, FeedbackName, 0));

    GL_CHECK(glBindVertexArray(VertexArrayName[buffer::FEEDBACK2]));
    GL_CHECK(glDrawTransformFeedbackStream(GL_POINTS, FeedbackName, 1));
}
