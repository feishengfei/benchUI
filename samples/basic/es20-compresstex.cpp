...
bool setupGraphics(int w, int h, GLenum format) {
    ...
    GL_CHECK(glCompressedTexImage2D(GL_TEXTURE_2D, 0, format, loadedETCHeader.getWidth(),loadedETCHeader.getHeight(), 0, loadedETCHeader.getPaddedWidth() * loadedETCHeader.getPaddedHeight() >> 1, textureData + 16));
    free(textureData);
...
void Sample::test() {
    for(int i=0;i<ARRAY_NUM(format);i++) {
        setupGraphics(WINDOW_W, WINDOW_H, format[i]);
        renderFrame();
        string format_name = get_enum_name(format[i]);
