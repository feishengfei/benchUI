 ...
 * @brief basic test for copytex1 related API
 * @par TestPoints:
 *
 */
...
void renderFrame(GLenum target, GLenum format, int n) {
    ...
    glCopyTexImage2D(target, 0, format, 0, 0, 256, 256, 0);
    GL_CHECK(glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST));

    glViewport(0,0,WINDOW_W,WINDOW_H);
    GL_CHECK(glClearColor(0.7f, 0.3f, 0.8f, 0.5));
    GL_CHECK(glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT));
...
void Sample::test() {
    for (int i = 0; i < ARRAY_NUM(target); i++) {
        for (int j = 0; j < ARRAY_NUM(format); j++) {
            setupGraphics(WINDOW_W, WINDOW_H);
            renderFrame(target[i], format[j], j);
            string target_name = get_enum_name(target[i]);
            string format_name = get_enum_name(format[j]);
            char ss[3];
            sprintf(ss, "%d%d", i, j);
            string bmp_file  = filename + "-" + target_name + "-" + format_name + ss + ".bmp";
