/**
 * @file accum2.c
 * @testpoint gl32-DrawElementsBaseVertex.cpp
 *
 */
#include "Sample.h"
#include "Primitive.h"
...
void renderFrame(GLenum mode, GLenum type)
{
    ...
    if(type == GL_UNSIGNED_BYTE)
        GL_CHECK(glDrawElementsBaseVertex(mode, NUM, type, indices_ub, 3));
    if(type == GL_UNSIGNED_SHORT)
        GL_CHECK(glDrawElementsBaseVertex(mode, NUM, type, indices_us, 3));
    if(type == GL_UNSIGNED_INT)
        GL_CHECK(glDrawElementsBaseVertex(mode, NUM, type, indices_ui, 3));
    GL_CHECK(glFlush());
}
