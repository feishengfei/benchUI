...
static GLushort images[iWidth][4];
static GLfloat imagef[iWidth][4];
static GLuint imagei[iWidth][4];
...
void makeImage(void) {
    int t;
        ...
        for (t = 0; t < iWidth; t++) {
            ...
            imageb[t][2] = (GLubyte)(t *17);
            imageb[t][3] = (GLubyte)(t * 0);
        }
        
        for (t = 0; t < iWidth; t++) {
            images[t][0] = (GLushort)(t * 0);
            images[t][1] = (GLushort)(t * 8);
            images[t][2] = (GLushort)(t *17);
            images[t][3] = (GLushort)(t * 0);
        }

  
        for (t = 0; t < iWidth; t++) {
            imagef[t][0] = (GLfloat)(t * 0);
            imagef[t][1] = (GLfloat)(t * 8);
            imagef[t][2] = (GLfloat)(t *17);
            imagef[t][3] = (GLfloat)(t * 0);
        }
        
        for (t = 0; t < iWidth; t++) {
            imagei[t][0] = (GLuint)(t * 0);
            imagei[t][1] = (GLuint)(t * 8);
            imagei[t][2] = (GLuint)(t *17);
            imagei[t][3] = (GLuint)(t * 0);
        }
}
...
bool setupGraphics(int width, int height, int i) {
    ...
    glBindTexture(GL_TEXTURE_1D, texBufferObject);
    glTexParameteri(GL_TEXTURE_1D, GL_TEXTURE_MIN_FILTER,GL_NEAREST);
    glTexParameteri(GL_TEXTURE_1D, GL_TEXTURE_MAG_FILTER,GL_NEAREST);
    glTexParameteri(GL_TEXTURE_1D, GL_TEXTURE_WRAP_S,GL_CLAMP_TO_EDGE);
    GL_CHECK(glTexParameteri(GL_TEXTURE_1D, GL_TEXTURE_WRAP_T,GL_CLAMP_TO_EDGE));
    if(i<21) {
        GL_CHECK(glTexStorage1D(GL_TEXTURE_1D, 1, factors[i].interfmt, iWidth));
        GL_CHECK(glTexSubImage1D(GL_TEXTURE_1D, 0, 0, iWidth,
                                 factors[i].format, factors[i].type,  DATA_TRANS(imageb)));
    } else if(i<32) {
        GL_CHECK(glTexStorage1D(GL_TEXTURE_1D, 1, factors[i].interfmt, iWidth));
        GL_CHECK(glTexSubImage1D(GL_TEXTURE_1D, 0, 0, iWidth,
                                 factors[i].format, factors[i].type,  DATA_TRANS(imagef)));
    } else if(i<40) {
        GL_CHECK(glTexStorage1D(GL_TEXTURE_1D, 1, factors[i].interfmt, iWidth));
        GL_CHECK(glTexSubImage1D(GL_TEXTURE_1D, 0, 0, iWidth,
                                 factors[i].format, factors[i].type,  DATA_TRANS(images)));
    } else if(i<54) {
        GL_CHECK(glTexStorage1D(GL_TEXTURE_1D, 1, factors[i].interfmt, iWidth));
        GL_CHECK(glTexSubImage1D(GL_TEXTURE_1D, 0, 0, iWidth,
                                 factors[i].format, factors[i].type, DATA_TRANS(imagei)));
    } else if(i<58) {
        GL_CHECK(glTexStorage1D(GL_TEXTURE_1D, 1, factors[i].interfmt, iWidth));
        GL_CHECK(glCompressedTexSubImage1D(GL_TEXTURE_1D, 0, factors[i].interfmt,
                                           iWidth, 5, sizeof(image)/2, DATA_TRANS(image)));
    } else {
        GL_CHECK(glTexStorage1D(GL_TEXTURE_1D, 1, factors[i].interfmt, iWidth));
        GL_CHECK(glCompressedTexSubImage1D(GL_TEXTURE_1D, 0, factors[i].interfmt,
                                           iWidth, 5, sizeof(image), DATA_TRANS(image)));
    }
...
void Sample::test() {
    ...
    //    Sample::SwapBuffers();
    //}
    setupGraphics(WINDOW_W, WINDOW_H, 29);
