...
/**
 * @file blendfuncsep.cpp
 * @author Otto.Liu
 * @par History:
 *    A001: Otto.Liu, 2019-08-26, Add commit
 * @brief basic test for blendfuncsep related API
 * @par TestPoints:
 *
 */
...
bool setupGraphics(int width, int height, GLenum src_factor_rgb,
                   GLenum dst_factor_rgb, GLenum src_factor_a, GLenum dst_factor_a) {
	...
	|| needs_blend_color(src_factor_a) || needs_blend_color(dst_factor_a)) {
            ...
            for(int m=0; m<ARRAY_NUM(factors); m++) {
                for (int n = 0; n < ARRAY_NUM(factors); n++) {
                    if (n == 10 || j == 10) {
                        ...
                        string bmp_file = filename + "-" + ss  + "-" + src_rgb_name
                                          + "-" + dst_rgb_name + "-" + src_a_name +
                                          "-"+ dst_a_name + ".bmp";
