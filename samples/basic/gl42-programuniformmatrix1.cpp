...
bool setupGraphics(int w, int h) {
    ...
    string vertexShaderPath   = ((string)GLSL_PATH) + "matrix1.vert";
    string fragmentShaderPath = ((string)GLSL_PATH) + "matrix1.frag";
...
void render(){
    ...
    GL_CHECK(glBindVertexArray(vao));
    GL_CHECK(glProgramUniformMatrix2fv(fs, 0, 1, GL_FALSE, pos));
    GL_CHECK(glProgramUniformMatrix3fv(fs, 1, 1, GL_FALSE, pos1));
    GL_CHECK(glProgramUniformMatrix4fv(fs, 4, 1, GL_FALSE, pos2));
    GL_CHECK(glProgramUniformMatrix2x3fv(fs, 8, 1, GL_FALSE, pos3));
    GL_CHECK(glProgramUniformMatrix3x2fv(fs, 10, 1, GL_FALSE, pos4));
    GL_CHECK(glProgramUniformMatrix2x4fv(fs, 12, 1, GL_FALSE, pos5));
    GL_CHECK(glProgramUniformMatrix4x2fv(fs, 14, 1, GL_FALSE, pos6));
    GL_CHECK(glProgramUniformMatrix3x4fv(fs, 16, 1, GL_FALSE, pos7));
    GL_CHECK(glProgramUniformMatrix4x3fv(fs, 19, 1, GL_FALSE, pos8));
