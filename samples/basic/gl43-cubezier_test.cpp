...
bool setupGraphics(int width, int height) {
    ...
    ShaderInfo  tess_shaders[] = {
            {GL_VERTEX_SHADER, "cubezier_test.vert", },
            {GL_TESS_CONTROL_SHADER, "cubezier_test.tcs", },
            {GL_TESS_EVALUATION_SHADER, "cubezier_test.tes", },
            {GL_FRAGMENT_SHADER, "cubezier_test.frag", },
            {GL_NONE,NULL},
    };
...
void renderFrame(void) {
  ...
  //  GL_CHECK(glUnmapBuffer(GL_ARRAY_BUFFER));
    ...
    GL_CHECK(glPatchParameteri(GL_PATCH_VERTICES, 32));
   ...
   // GL_CHECK(glUniformMatrix4fv(1, 1, GL_FALSE, perspective.getAsArray()));
    ...
    GL_CHECK(glDrawArrays(GL_PATCHES, 0, 32));
 ...
 //   GL_CHECK(glDrawArrays(GL_LINES, 0, 32));
