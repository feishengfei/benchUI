...
GLfloat  laplacian[3] = { -0.125,  1.0  , -0.125 };
...
void getdata(int num)
{
	...
	glGetConvolutionFilter(GL_CONVOLUTION_1D, factors[num].format, factors[num].type, paramf);
	glGetConvolutionParameteriv(GL_CONVOLUTION_1D, GL_CONVOLUTION_BORDER_MODE, &parami);
	LOGIF("GetConvolutionParameteriv(GL_CONVOLUTION_BORDER_MODE)=%d\n",parami);
	glGetConvolutionParameterfv(GL_CONVOLUTION_1D, GL_CONVOLUTION_BORDER_COLOR, paramf);
	LOGIF("GetConvolutionParameterfv(GL_CONVOLUTION_BORDER_COLOR)=%f %f %f %f\n",paramf[0],paramf[1],paramf[2],paramf[3]);
	glGetConvolutionParameterfv(GL_CONVOLUTION_1D, GL_CONVOLUTION_FILTER_SCALE, paramf);
	LOGIF("GetConvolutionParameterfv(GL_CONVOLUTION_FILTER_SCALE)=%f %f %f %f\n",paramf[0],paramf[1],paramf[2],paramf[3]);
	glGetConvolutionParameterfv(GL_CONVOLUTION_1D, GL_CONVOLUTION_FILTER_BIAS, paramf);
	LOGIF("GetConvolutionParameterfv(GL_CONVOLUTION_FILTER_BIAS)=%f %f %f %f\n",paramf[0],paramf[1],paramf[2],paramf[3]);
	glGetConvolutionParameteriv(GL_CONVOLUTION_1D, GL_CONVOLUTION_FORMAT, &parami);
	LOGIF("GetConvolutionParameteriv(GL_CONVOLUTION_FORMAT)=%d\n",parami);
	glGetConvolutionParameterfv(GL_CONVOLUTION_1D, GL_CONVOLUTION_WIDTH, paramf);
	LOGIF("GetConvolutionParameterfv(GL_CONVOLUTION_WIDTH)=%f\n",paramf[0]);
	glGetConvolutionParameterfv(GL_CONVOLUTION_1D, GL_CONVOLUTION_HEIGHT, paramf);
	LOGIF("GetConvolutionParameterfv(GL_CONVOLUTION_HEIGHT)=%f\n",paramf[0]);
	glGetConvolutionParameterfv(GL_CONVOLUTION_1D, GL_MAX_CONVOLUTION_WIDTH, paramf);
	LOGIF("GetConvolutionParameterfv(GL_MAX_CONVOLUTION_WIDTH)=%f\n",paramf[0]);
	glGetConvolutionParameterfv(GL_CONVOLUTION_1D, GL_MAX_CONVOLUTION_HEIGHT, paramf);
...
{    
   ...
   glConvolutionFilter1D(GL_CONVOLUTION_1D, factors[num].internalfmt, 3, factors[num].format, factors[num].type, laplacian);
   glEnable(GL_CONVOLUTION_2D);
   glConvolutionParameteri(GL_CONVOLUTION_1D, GL_CONVOLUTION_BORDER_MODE, GL_REDUCE);
   glConvolutionParameteri(GL_CONVOLUTION_1D, GL_CONVOLUTION_BORDER_MODE, GL_CONSTANT_BORDER);
   glConvolutionParameteri(GL_CONVOLUTION_1D, GL_CONVOLUTION_BORDER_MODE, GL_REPLICATE_BORDER);
   glConvolutionParameterfv(GL_CONVOLUTION_1D, GL_CONVOLUTION_BORDER_COLOR, color);
   glConvolutionParameterfv(GL_CONVOLUTION_1D, GL_CONVOLUTION_FILTER_SCALE, scale);
   glConvolutionParameterfv(GL_CONVOLUTION_1D, GL_CONVOLUTION_FILTER_BIAS, bias);
   glCopyConvolutionFilter1D(GL_CONVOLUTION_1D, factors[num].internalfmt, 0, 0, 4);
