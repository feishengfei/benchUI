...
bool setupGraphics(int w, int h) {
    ...
    string fragmentShaderPath = ((string)GLSL_PATH) + "dpipeline1.frag";
...
void render(){
    ...
    GL_CHECK(glProgramUniform4d(fs, 0, 1.0, 0.0, 0.0, 1.0));
    GL_CHECK(glDrawArrays(GL_TRIANGLES, 0, 3));
    const GLdouble fv[4] = {1.0, 1.0, 0.0, 1.0};
    GL_CHECK(glBindVertexArray(vao[1]));
    GL_CHECK(glStencilFunc(GL_NOTEQUAL, 0x1, 0xff));
    GL_CHECK(glStencilOp(GL_KEEP, GL_KEEP, GL_KEEP));
    GL_CHECK(glStencilMask(0x0));
    GL_CHECK(glProgramUniform4dv(fs, 0, 1, fv));
    GL_CHECK(glDrawArrays(GL_TRIANGLES, 0, 3));
}
