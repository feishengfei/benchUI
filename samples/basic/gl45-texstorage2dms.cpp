...
bool setupGraphics(int w, int h, int sampleNum) {
    ...
    GLint buffers;
    GL_CHECK(glGetIntegerv(GL_SAMPLE_BUFFERS, &buffers));
    LOGI("SAMPLE BUFFER IS %d\n", buffers);
    ...
    GLint pname;
    GL_CHECK(glGetInternalformativ(GL_RENDERBUFFER, GL_RGBA8, GL_NUM_SAMPLE_COUNTS, 1, &pname));
    LOGI("pname = %d\n", pname);
