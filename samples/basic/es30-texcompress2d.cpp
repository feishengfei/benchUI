...
/**
 * @file texcompress2d.cpp
 * @author Otto.Liu
 * @par History:
 *    A001: Otto.Liu, 2019-09-02, Add commit
 * @brief basic test for texcompress2d related API
 * @par TestPoints:
 *
 */
...
bool setupGraphics(int width, int height, int i) {
    ...
    if(i<4) {
    GL_CHECK(glCompressedTexImage2D(GL_TEXTURE_2D, 0, formats[i], 32, 32,
									0, sizeof(image)/2, image));
    } else {
    GL_CHECK(glCompressedTexImage2D(GL_TEXTURE_2D, 0, formats[i], 64, 64,
									0, sizeof(image), image));
    }
