...
bool test_object_label()
{
    ...
    GL_CHECK(glObjectLabel(GL_PROGRAM, program, -1, TestLabel));
    GL_CHECK(glGetObjectLabel(GL_PROGRAM, program, TestLabelLen + 1, &length[PROGRAM_IDX],
                     label[PROGRAM_IDX]));
    ...
    GL_CHECK(glObjectLabel(GL_BUFFER, buffer, -1, TestLabel));
    GL_CHECK(glGetObjectLabel(GL_BUFFER, buffer, TestLabelLen + 1,
    ...
    GL_CHECK(glObjectLabel(GL_TEXTURE, texture, -1, TestLabel));
    GL_CHECK(glGetObjectLabel(GL_TEXTURE, texture, TestLabelLen + 1, &length[TEXTURE_IDX],
                     label[TEXTURE_IDX]));
...
bool test_object_ptr_label()
{
	...
	GL_CHECK(glObjectPtrLabel(sync, -1, TestLabel));
	GL_CHECK(glGetObjectPtrLabel(sync, TestLabelLen + 1, &length, label));
    ...
    GL_CHECK(glObjectPtrLabel(sync, length, label));
    glDeleteSync(sync);
