 ...
 * @brief basic test for pointsprite related API
 * @par TestPoints:
 *
 */
...
void renderFrame(GLenum factor) {
	...
	GL_CHECK(glEnable(GL_POINT_SPRITE_OES));//
	...
	glTexEnvi(GL_POINT_SPRITE_OES, GL_COORD_REPLACE_OES, factor);//GL_FALSE

	glBindTexture(GL_TEXTURE_2D, texName);
	glMultiTexCoord4f(GL_TEXTURE0, 1.0, 0.0, 0.0, 1.0);
	GL_CHECK(glPointSize(20.0));
	GLfloat vert[3] = {0.0, 0.0, 0.0};
	GL_CHECK(glEnableClientState(GL_VERTEX_ARRAY));
	GL_CHECK(glVertexPointer(3, GL_FLOAT, 0, vert));
	GL_CHECK(glDrawArrays(GL_POINTS, 0, 1));
	GLint param;
	glGetTexEnviv(GL_POINT_SPRITE_OES, GL_COORD_REPLACE_OES, &param);
	LOGI("GetTexEnv(GL_POINT_SPRITE_OES, GL_COORD_REPLACE_OES): %d\n", param);
	GL_CHECK(glFlush());
	GL_CHECK(glDisable(GL_TEXTURE_2D));
	GL_CHECK(glDisable(GL_DEPTH_TEST));
	GL_CHECK(glDisable(GL_POINT_SPRITE_OES));
...
void Sample::test()
{
	...
	for(int i=0; i<2; i++) {
		...
		string factor_name = GET_BOOL(factors[i]);
