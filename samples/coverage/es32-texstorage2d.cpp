...
string vertexShaderFilename = "teximage2d.vert";
string fragmentShaderFilename = "teximage2d.frag";
...
static GLushort images[iHeight][iWidth][4];
static GLfloat imagef[iHeight][iWidth][4];
static GLuint imagei[iHeight][iWidth][4];
...
void makeImage(void) {
	...
	for (s = 0; s < iHeight; s++)
		for (t = 0; t < iWidth; t++) {
			...
			imageb[t][s][1] = (GLubyte)(t * 17);
			imageb[t][s][2] = (GLubyte)(r * 17);
			imageb[t][s][3] = (GLubyte)(r * 17);
		}

	for (s = 0; s < iHeight; s++)
		for (t = 0; t < iWidth; t++) {
			images[t][s][0] = (GLushort)(s * 17);
			images[t][s][1] = (GLushort)(t * 17);
			images[t][s][2] = (GLushort)(r * 17);
			images[t][s][3] = (GLushort)(r * 17);
		}
	...
	for (s = 0; s < iHeight; s++)
		for (t = 0; t < iWidth; t++) {
			...
			imagef[t][s][2] = (GLfloat)(r * 17);
			imagef[t][s][3] = (GLfloat)(r * 17);
		}

	for (s = 0; s < iHeight; s++)
		for (t = 0; t < iWidth; t++) {
			imagei[t][s][0] = (GLuint)(s * 17);
			imagei[t][s][1] = (GLuint)(t * 17);
			imagei[t][s][2] = (GLuint)(r * 17);
			imagei[t][s][3] = (GLuint)(r * 17);
		}
}
...
bool setupGraphics(int width, int height, int i) {
    ...
    if(i<21) {
    GL_CHECK(glTexStorage2D(GL_TEXTURE_2D, 1, factors[i].interfmt, iWidth, iHeight));
    GL_CHECK(glTexSubImage2D(GL_TEXTURE_2D, 0, 0,0, iWidth, iHeight,
							 factors[i].format, factors[i].type, imageb));
    } else if(i<32) {
    GL_CHECK(glTexStorage2D(GL_TEXTURE_2D, 1, factors[i].interfmt, iWidth, iHeight));
    GL_CHECK(glTexSubImage2D(GL_TEXTURE_2D, 0, 0,0, iWidth, iHeight,
							 factors[i].format, factors[i].type, imagef));
    } else if(i<40) {
    GL_CHECK(glTexStorage2D(GL_TEXTURE_2D, 1, factors[i].interfmt, iWidth, iHeight));
    GL_CHECK(glTexSubImage2D(GL_TEXTURE_2D, 0, 0,0, iWidth, iHeight,
							 factors[i].format, factors[i].type, images));
    } else if(i<54) {
    GL_CHECK(glTexStorage2D(GL_TEXTURE_2D, 1, factors[i].interfmt, iWidth, iHeight));
    GL_CHECK(glTexSubImage2D(GL_TEXTURE_2D, 0, 0,0, iWidth, iHeight,
							 factors[i].format, factors[i].type, imagei));
    } else if(i<58) {
    GL_CHECK(glTexStorage2D(GL_TEXTURE_2D, 1, factors[i].interfmt, 32,32));
    GL_CHECK(glCompressedTexSubImage2D(GL_TEXTURE_2D, 0, 0,0, 32,32,
									   factors[i].interfmt, sizeof(image)/2, image));
    } else {
    GL_CHECK(glTexStorage2D(GL_TEXTURE_2D, 1, factors[i].interfmt, 64,64));
    GL_CHECK(glCompressedTexSubImage2D(GL_TEXTURE_2D, 0, 0,0, 64,64,
