...
#define COLOR_TEXTURE_NAME1 ("5.bmp")
#define COLOR_TEXTURE_NAME2 ("mali.bmp")
...
bool setupGraphics(int w, int h) {
    ...
    string texturePath2 = ((string)PICTURE_PATH) + COLOR_TEXTURE_NAME2;
    string vertexShaderPath = ((string)GLSL_PATH) + "textureview.vert";
    string fragmentShaderPath = ((string)GLSL_PATH) + "textureview.frag";
    ...
    GL_CHECK(glGenTextures(1, &dstTex));
    GL_CHECK(glBindTexture(GL_TEXTURE_2D, dstTex));
    Texture::loadBmpImageData(texturePath2.c_str(), &imageWidth, &imageHeight, &textureData2);
    GL_CHECK(glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, imageWidth, imageHeight, 0, GL_RGB,
                          GL_UNSIGNED_BYTE, textureData2));
    GL_CHECK(glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST));

    GL_CHECK(glCopyImageSubData(srcTex, GL_TEXTURE_2D, 0, 0, 0, 0, dstTex, GL_TEXTURE_2D,
                                0, 0, 0, 0, 300, 256, 1));
...
void renderFrame(void) {
    ...
    GL_CHECK(glDeleteTextures(1, &dstTex));
}
