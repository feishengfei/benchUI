...
void fillData()
{
    GL_CHECK(glProgramUniform1ui(fs, glGetUniformLocation(fs, "f1"), 1));
    GL_CHECK(glProgramUniform2ui(fs, glGetUniformLocation(fs, "f2"), 1, 1));
    GL_CHECK(glProgramUniform3ui(fs, glGetUniformLocation(fs, "f3"), 1, 1, 1));
    GL_CHECK(glProgramUniform4ui(fs, glGetUniformLocation(fs, "f4"), 1, 1, 1, 1));

    const GLuint fv1[1] = {1};
    const GLuint fv2[2] = {1, 1};
    const GLuint fv3[3] = {1, 1, 1};
    const GLuint fv4[4] = {1, 1, 1, 1};

    GL_CHECK(glProgramUniform1uiv(fs, glGetUniformLocation(fs, "fv1"), 1, fv1));
    GL_CHECK(glProgramUniform2uiv(fs, glGetUniformLocation(fs, "fv2"), 1, fv2));
    GL_CHECK(glProgramUniform3uiv(fs, glGetUniformLocation(fs, "fv3"), 1, fv3));
    GL_CHECK(glProgramUniform4uiv(fs, glGetUniformLocation(fs, "fv4"), 1, fv4));
/*
...
bool setupGraphics(int w, int h) {
    ...
    string vertexShaderPath   = ((string)GLSL_PATH) + "glProgramUniformxui_uiv.vert";
    string fragmentShaderPath = ((string)GLSL_PATH) + "glProgramUniformxui_uiv.frag";
