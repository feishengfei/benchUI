...
/**
 * @file triangle_frame_buffer.cpp
 * @author Otto.Liu
 * @par History:
 *    A001: Otto.Liu, 2019-08-26, Add commit
 * @brief basic test for triangle_frame_buffer related API
 * @par TestPoints:
 *
 */
...
bool setupGraphics(int width, int height) {
    ...
    string vertexShaderPath   = ((string)GLSL_PATH) + "mixtexture.vert";
    string fragmentShaderPath = ((string)GLSL_PATH) + "mixtexture.frag";
    ...
    glGetFramebufferAttachmentParameteriv(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_FRAMEBUFFER_ATTACHMENT_TEXTURE_LEVEL,
                                          &param);
    LOGI("GL_FRAMEBUFFER_ATTACHMENT_TEXTURE_LEVEL=%d\n", param);
    glGetFramebufferAttachmentParameteriv(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0,
                                          GL_FRAMEBUFFER_ATTACHMENT_TEXTURE_CUBE_MAP_FACE, &param);
    ...
    LOGI("GL_FRAMEBUFFER_ATTACHMENT_OBJECT_TYPE=%d\n", param);
    GLint parami;
    glGetIntegerv(GL_BLUE_BITS, &parami);
    LOGI("get(GL_BLUE_BITS): %d", parami);
    glGetIntegerv(GL_DEPTH_BITS, &parami);
    LOGI("get(GL_DEPTH_BITS): %d", parami);
    glGetIntegerv(GL_GREEN_BITS, &parami);
    LOGI("get(GL_GREEN_BITS): %d", parami);
    glGetIntegerv(GL_FRAMEBUFFER_BINDING, &parami);
    LOGI("get(GL_FRAMEBUFFER_BINDING): %d", parami);
    glGetIntegerv(GL_IMPLEMENTATION_COLOR_READ_FORMAT, &parami);
    LOGI("get(GL_IMPLEMENTATION_COLOR_READ_FORMAT): %d", parami);
    glGetIntegerv(GL_IMPLEMENTATION_COLOR_READ_TYPE, &parami);
    LOGI("get(GL_IMPLEMENTATION_COLOR_READ_TYPE): %d", parami);
    glGetIntegerv(GL_RED_BITS, &parami);
    LOGI("get(GL_RED_BITS): %d", parami);
    glGetIntegerv(GL_STENCIL_BITS, &parami);
    LOGI("get(GL_STENCIL_BITS): %d", parami);
