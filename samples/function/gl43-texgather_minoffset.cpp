...
void loadShader(void)
{
   ...
   std::string fragmentShaderSource_s =
        ...
        "void main()\n"
        "{\n"
        "FragColor=textureGatherOffset(sampler, texcoord, ivec2("+std::to_string(-min_offset)+", "+std::to_string(min_offset)+"));\n}";
...
{
    GL_CHECK(glGetIntegerv(GL_MIN_PROGRAM_TEXTURE_GATHER__OFFSET, &min_offset));
    LOGI("GL_MIN_PROGRAM_TEXTURE_GATHER__OFFSET = %d\n", min_offset);
