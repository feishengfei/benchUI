...
void loadShader(void)
{
   ...
   std::string fragmentShaderSource0 =
    ...
    for(int i=0; i<max_image_units; i++){
        ...
        fragmentShaderSource_col.append("vec4 color"+std::to_string(i)+"=texture(sampler"+std::to_string(i)+", texcoord);\n");
...
{
    GL_CHECK(glGetIntegerv(GL_MAX_TEXTURE_IMAGE_UNITS, &max_image_units));
