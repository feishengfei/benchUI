...
void loadShader(void) {
    GLint  success;
    GLchar infoLog[512];
    vertex_num=max_total_output_components/max_output_components;
    ...
    std::string geometryShaderSource0 =
                        "#version 430 core\n"
                        "layout(points) in;\n"
                        "layout(points, max_vertices="+std::to_string(vertex_num)+") out;\n";

    std::string geometryShaderSourcehead =  "out GS_OUT {\n";
    ...
    std::string geometryShaderSource_uni;
    for(int i=0; i<vertex_num; i++){
        geometryShaderSource_uni.append("layout(location="+std::to_string(i)+") uniform vec4 mv"+std::to_string(i)+";\n");
        geometryShaderSource_main.append("gl_Position = gl_in[0].gl_Position+mv"+std::to_string(i)+";\n" + color_component_out);
    ...
    geometryShaderSource_s.append(geometryShaderSource_uni);
    geometryShaderSource_s.append(geometryShaderSourcehead);
...
void renderFrame(void){
    ...
    for(int i=0; i<vertex_num; i++){
        GL_CHECK(glUniform4f(i, 2.0*i/vertex_num, 2.0*i/vertex_num, 0.0, 0.0));
    }
    GL_CHECK(glPointSize(3.0));
    GL_CHECK(glBindVertexArray(VAOs));
    GL_CHECK(glDrawArrays(GL_POINTS, 0, vertex_num));
}
