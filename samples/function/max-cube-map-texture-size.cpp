 ...
 *
 * @file mt_imageLookupFuncs_3_1.cpp
 * @author Martin.li
 * @par History:
...
/*
    double n = pow(my_cube_map_texture_size, 1.0/3);
    int my_cube_map_texture_size_int = floor(n);
    LOGI(" nnnnnnnnnnnnnnn = %f;  %d\n ", n, my_cube_map_texture_size_int);
*/
    const int step = 4;
    const int width = 4;
    const int height = 4;
    ...
    GL_CHECK(glTexStorage3D(GL_TEXTURE_CUBE_MAP_ARRAY, 1, GL_R32F, my_cube_map_texture_size, my_cube_map_texture_size, my_cube_map_texture_size * 6));
//    GL_CHECK(glTexSubImage3D(GL_TEXTURE_CUBE_MAP_ARRAY, 0, 0, 0, 0, width, height, depth * 6, GL_RED, GL_FLOAT, image_CubeArray));
//    GL_CHECK(glTexSubImage3D(GL_TEXTURE_CUBE_MAP_ARRAY, 0, 0, 0, 0, width, height, depth * 6, GL_RED, GL_FLOAT, image_CubeArray));

    for(int i = 0 ; i < my_cube_map_texture_size/step ; i++)
    {
        static int temp = i * step;
        GL_CHECK(glTexSubImage3D(GL_TEXTURE_CUBE_MAP_ARRAY, 0, temp, temp, temp, width, height, depth * 6, GL_RED, GL_FLOAT, image_CubeArray));
...
{
#ifdef API_ES
    string path = "es32_imageLookupFuncs_3_1";
#else
    string path = "gl43_imageLookupFuncs_3_1";
    GL_CHECK(glEnable(GL_PROGRAM_POINT_SIZE));
#endif
    string vertexShaderPath = (string)GLSL_PATH + "max-cube-map-texture-size.vert";
    string fragmentShaderPath = (string)GLSL_PATH + "max-cube-map-texture-size.frag";
