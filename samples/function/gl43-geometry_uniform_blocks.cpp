...
void loadShader(void)
{
    ...
    geometryShaderSource_uniform0.append("layout(std140, binding=0) uniform MVP{\n mat4 rotation;\n};\n");
    ...
    for(int i=1; i<max_uniform_blocks; i++){
       geometryShaderSource_uniform1.append("layout(std140, binding="+std::to_string(i)+") uniform MVP"+std::to_string(i)+"{\nmat4 scaling"+std::to_string(i)+";\n}ubo"+std::to_string(i)+";\n");
       std::string temp;
...
{
    GL_CHECK(glGetIntegerv(GL_MAX_GEOMETRY_UNIFORM_BLOCKS, &max_uniform_blocks));
...
void renderFrame(void) {
    ...
    memcpy(blockBuffer[0], scale0.getAsArray(), 16*sizeof(float));
    for(int i=1; i<max_uniform_blocks-1; i++){
        memcpy(blockBuffer[i], scale0.getAsArray(), 16*sizeof(float));
    }
    memcpy(blockBuffer[max_uniform_blocks-1], scale1.getAsArray(), 16*sizeof(float));
