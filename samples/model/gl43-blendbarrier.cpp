...
void renderFrame(void) {
    ...
    GL_CHECK(glBlendEquationi(0, GL_FUNC_ADD));
    GL_CHECK(glBlendFunci(0, GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA));
    LOGI("IsEnablei(GL_BLEND, 0): %d\n", glIsEnabledi(GL_BLEND, 0));
    GL_CHECK(glDrawArrays(GL_TRIANGLES, 0, 3));

    GL_CHECK(glDisablei(GL_BLEND, 0));
    GL_CHECK(glBlendEquationSeparatei(0, GL_FUNC_ADD, GL_FUNC_ADD));
    GL_CHECK(glBlendFuncSeparatei(0, GL_SRC_ALPHA, GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA,
                                  GL_ONE_MINUS_SRC_ALPHA));
    LOGI("IsEnablei(GL_BLEND, 0): %d\n", glIsEnabledi(GL_BLEND, 0));
    GL_CHECK(glDrawArrays(GL_TRIANGLES, 3, 3));

    GL_CHECK(glEnablei(GL_BLEND, 1));
    GL_CHECK(glBlendEquationi(0, GL_FUNC_ADD));
    GL_CHECK(glBlendFunci(0, GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA));
    GL_CHECK(glDrawArrays(GL_TRIANGLES, 6, 3));

   // GL_CHECK(glBlendBarrier());
}
