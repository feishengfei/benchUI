...
float pointSize[1500];
float pointVertices[3000];

bool setupGraphics(int w, int h) {
    string vertexShaderPath   = ((string)GLSL_PATH) + "multipoints.vert";
...
void render(){
    ...
    GL_CHECK(glBufferData(GL_ARRAY_BUFFER, sizeof(pointVertices) + sizeof(pointSize), NULL, GL_STATIC_DRAW));
    GL_CHECK(glBufferSubData(GL_ARRAY_BUFFER, 0, sizeof(pointVertices), pointVertices));
    GL_CHECK(glBufferSubData(GL_ARRAY_BUFFER, sizeof(pointVertices), sizeof(pointSize), pointSize));
    GL_CHECK(glEnableVertexAttribArray(0));
    GL_CHECK(glVertexAttribPointer(0, 2, GL_FLOAT, GL_FALSE, 0, 0));
    GL_CHECK(glEnableVertexAttribArray(1));
    GL_CHECK(glVertexAttribPointer(1, 1, GL_FLOAT, GL_FALSE, 0, BUFFER_OFFSET(sizeof(pointVertices))));

    GL_CHECK(glBindVertexArray(VAOs));
    GL_CHECK(glDrawArrays(GL_TRIANGLE_FAN, 0, 1500));
}
