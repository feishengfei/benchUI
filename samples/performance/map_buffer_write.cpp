...
GLuint                    vao, vaob, vbo, vbob, programID;
...
const GLchar *fragmentShaderSource =
    ...
    renderFrame() {
        ...
        GLfloat *q;
        q = (GLfloat *)glMapBufferRange(GL_ARRAY_BUFFER, 0, sizeof(vertices),
                                        GL_MAP_WRITE_BIT);
        memcpy(q, vertices, sizeof(vertices));
        GL_CHECK(glUnmapBuffer(GL_ARRAY_BUFFER));
        GLfloat *p;
        p = (GLfloat *)glMapBufferRange(GL_ARRAY_BUFFER, sizeof(vertices), sizeof(colors),
                                        GL_MAP_WRITE_BIT);
...
void Sample::test() {
    ...
    LOGI("map_buffer_write One frame average time:--%f\n", ave_time);
}
