...
bool initProgram()
    ...
    void renderFrame() {
        ...
        GL_CHECK(glBeginQuery(GL_SAMPLES_PASSED, query));
        GL_CHECK(glDrawArrays(GL_TRIANGLES, 0, VERTEX_NUM-3));
        GL_CHECK(glEndQuery(GL_SAMPLES_PASSED));

        GL_CHECK(glBeginConditionalRender(query, GL_QUERY_WAIT));
        GL_CHECK(glDrawArrays(GL_TRIANGLES, VERTEX_NUM-3, 3));
