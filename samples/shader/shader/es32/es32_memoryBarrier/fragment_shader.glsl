...
layout (binding = 0, r32f)  uniform volatile highp image2D imageUnit0;
...
{		
	...
	ivec4 col_1;
	if(gl_PrimitiveID == 0){
/*					col_0 = imageLoad(imageUnit0, ivec2(1, 0));
					memoryBarrier();
...
*/
					...
					for(int i = 0; i <= 10000; i++){
						imageStore(imageUnit0, ivec2(1, 0), vec4(float(i), 0.0, 0.0, 0.0));
					}
//					memoryBarrier();
					vec4 col_0 = imageLoad(imageUnit0, ivec2(1, 0));
					results[0] = (col_0.x == 10000.0);
	}
	if(gl_PrimitiveID == 1){
/*					col_1 = imageLoad(imageUnit1, ivec2(1, 0));
					memoryBarrier();
