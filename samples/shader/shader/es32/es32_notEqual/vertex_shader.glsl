...
void main()
{
	...
	bvec2 v2 = notEqual(c2, r2);
	bvec3 v3 = notEqual(c3, r3);
	bvec4 v4 = notEqual(c4, r4);
	...
	bvec2 iv2 = notEqual(ic2, ir2);
	bvec3 iv3 = notEqual(ic3, ir3);
	bvec4 iv4 = notEqual(ic4, ir4);
	...
	bvec2 uv2 = notEqual(uc2, ur2);
	bvec3 uv3 = notEqual(uc3, ur3);
	bvec4 uv4 = notEqual(uc4, ur4);
	...
	bvec2 bv2 = notEqual(bc2, br2);
	bvec3 bv3 = notEqual(bc3, br3);
	bvec4 bv4 = notEqual(bc4, br4);

	float average_b = (float(bv2.y) + float(bv3.y) + float(bv4.y)) / 3.0;
	
	float average = average_v + average_i + average_u + average_b;

	gl_Position = vec4(vPosition.x / 10.0, (4.0 - average) / 3.0, vPosition.z, 1.0);   
																						///average is number of "notEqual"
																						///(4.0 - average) is number of "equal" 
	vfColor = vColor;
	gl_PointSize = 8.0;
}
