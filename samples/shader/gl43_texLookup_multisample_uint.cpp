 ...
 *
 * @file mt_texLookup_multisample_uint.cpp
 * @author Martin.li
 * @par History:
...
///===================================================================================
    ...
    GLuint colors_bone[NumPoints_bone*4] = {
        8, 5, 1, 10,
        8, 5, 1, 10,
        8, 5, 1, 10,
...
void initVAO()
{
    ...
    GLuint loc1_bone = GL_CHECK(glGetAttribLocation(program_tex, "vPosition"));
    GL_CHECK(glEnableVertexAttribArray(loc1_bone));
    GL_CHECK(glVertexAttribPointer(loc1_bone, 3, GL_FLOAT, GL_FALSE, 0, BUFFER_OFFSET(0))); 
    GLuint loc2_bone = GL_CHECK(glGetAttribLocation(program_tex, "vColor"));
    GL_CHECK(glEnableVertexAttribArray(loc2_bone));
    GL_CHECK(glVertexAttribIPointer(loc2_bone, 4, GL_UNSIGNED_INT, 0, BUFFER_OFFSET(sizeof(points_bone))));
...
void initTexture()
{
    ...
    GL_CHECK(glGenTextures(1, &texture_ob_2DMS));
    GL_CHECK(glBindTexture(GL_TEXTURE_2D_MULTISAMPLE, texture_ob_2DMS));
    GL_CHECK(glTexStorage2DMultisample(GL_TEXTURE_2D_MULTISAMPLE, 4, GL_RGBA32UI, 4, 4, GL_TRUE));

    GL_CHECK(glGenFramebuffers(1, &fbo));
    GL_CHECK(glBindFramebuffer(GL_FRAMEBUFFER, fbo));
    GL_CHECK(glFramebufferTexture(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, texture_ob_2DMS, 0));
...
#ifdef API_ES
    string path = "es32_texLookup_multisample_uint";
#else
    string path = "gl43_texLookup_multisample_uint";
...
void renderFrame(void)
{
    ...
    GLuint tex0_uniform_loc = glGetUniformLocation(program, "texUnit0");
